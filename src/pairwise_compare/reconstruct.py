"""Reconstruct a full four-image order from pair probabilities."""
...
def normalise_pair_probabilities(pair_probabilities: Mapping[object, float]) -> dict[tuple[int, int], float]:
    ...
    for raw_key, raw_probability in pair_probabilities.items():
        ...
        if key in normalised and not math.isclose(normalised[key], canonical_probability, abs_tol=1e-6):
            raise ValueError(f"Conflicting probabilities supplied for pair {key}.")
        normalised[key] = canonical_probability
    return normalised


def probability_before(pair_probabilities: Mapping[tuple[int, int], float], first: int, second: int) -> float:
    if first < second:
        return pair_probabilities[(first, second)]
    return 1.0 - pair_probabilities[(second, first)]


def score_order(order: Sequence[int], pair_probabilities: Mapping[tuple[int, int], float], *, epsilon: float = 1e-7) -> float:
    score = 0.0
    for left_position in range(len(order)):
        for right_position in range(left_position + 1, len(order)):
            probability = probability_before(pair_probabilities, order[left_position], order[right_position])
            probability = min(max(probability, epsilon), 1.0 - epsilon)
            score += math.log(probability)
    return score


def reconstruct_best_order(pair_probabilities: Mapping[object, float], *, num_items: int = 4) -> tuple[list[int], float]:
    ...
    best_order, best_score = max(
        ((list(order), score_order(order, probabilities)) for order in permutations(range(1, num_items + 1))),
        key=lambda item: item[1],
    )
    return best_order, best_score


def order_to_submission_answer(order: Sequence[int]) -> list[int]:
...
def reconstruct_dataframe(pair_dataframe: pd.DataFrame, *, probability_column: str = "probability") -> pd.DataFrame:
    required = {"sample_id", "image_a_index", "image_b_index", probability_column}
    ...
    for sample_id, group in pair_dataframe.groupby("sample_id", sort=False):
        pair_probabilities = {
            (int(row.image_a_index), int(row.image_b_index)): float(getattr(row, probability_column))
        ...
        records.append(
            {
                "Id": str(sample_id),
                "order": str(order),
                "Answer": str(order_to_submission_answer(order)),
                "order_score": score,
            }
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Reconstruct full image orders from pair probabilities.")
...
def main() -> None:
    args = parse_args()
    pair_dataframe = pd.read_csv(args.input)
    reconstructed = reconstruct_dataframe(pair_dataframe, probability_column=args.probability_column)
